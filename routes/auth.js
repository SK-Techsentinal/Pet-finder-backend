...
function sendTokenResponse(user, statusCode, res) {
  ...
  const safeUser = user.toObject({ virtuals: true });
...
router.post('/register', async (req, res) => {
  try {
    ...
    // Start 7-day trial immediately on registration
...
router.post('/login', async (req, res) => {
  try {
    ...
    // Select password field explicitly (excluded by default in schema)
...
/**
 * Save or update the logged-in user's GPS location.
 * Called by the frontend after login when GPS is available.
 * This is what enables the Alert Neighbors feature to find
 * users near a lost pet.
 *
